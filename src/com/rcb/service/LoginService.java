...
public class LoginService {
	...
	public static boolean authenticate(User user) {
		try {
			...
			String sql="Select * from tbl_user where user_name='"+user.getUser_name()+"' and user_password='"+user.getUser_password()+"'";
	...
	public String getUserType(User user) {
		String user_type_db="";
		try {
			String selectUsertypeSql="SELECT tbl_account_type.type_name FROM tbl_account_type Inner Join tbl_user ON tbl_account_type.type_id = tbl_user.type_id where  user_name='"+user.getUser_name()+"'";
			ResultSet rs=db.getData(selectUsertypeSql);
			while (rs.next()) {
				user_type_db=rs.getString("type_name");
				System.out.println(user_type_db);
				return user_type_db;
			}
